...
def entropy(target_col):
    ...
    for y in target_uniq:
        Py = w[np.where(target_col == y)].sum()
...
def InfoGain(data, split_attribute_name, w, target_name="class"):
    """
...
def DecisionTree(data, features, depth, maxdepth, w, target_attribute_name="class"):
    """
    ...
    """
    neg_weight = w[np.where(data[target_attribute_name] == 0)].sum()
    pos_weight = w[np.where(data[target_attribute_name] == 1)].sum()
    ...
    if depth == maxdepth or np.unique(data[target_attribute_name]).size == 1 or len(features) == 0:
        return Node(pred, None, None, None, None)
    # If none of the above holds true, grow the tree!
    # First, select the feature which best splits the dataset
    max_gain_col = ''
    max_gain = float("-inf")
    for col in features:
        info_gain = InfoGain(data, col, w, target_attribute_name)
    ...
    if left.shape[0] != 0:
        left_node = DecisionTree(left, features, depth + 1, maxdepth, w)
    else:
        left_node = Node(pred, None, None, None, None)
    if right.shape[0] != 0:
        right_node = DecisionTree(right, features, depth + 1, maxdepth, w)
    else:
        right_node = Node(pred, None, None, None, None)
...
def random_forest(m, dmax, w, T=50):
    alpha = np.array([])
    train_acc = []
    valid_acc = []
    train_pred_his = np.empty((0, train.shape[0]))
    valid_pred_his = np.empty((0, valid.shape[0]))
    for t in tqdm(range(T)):
        dt = DecisionTree(train, random.sample(list(train.columns[:-1]), m), 0, dmax, w)
        ...
        error_weight = w[np.where(train_pred != train.iloc[:, -1])].sum()

        if error_weight == 0:
            error_weight = 1e-10

        print(error_weight)
        alpha = np.append(alpha, 0.5 * math.log((1 - error_weight) / error_weight))
        print(alpha[t])
        print(math.exp(-alpha[t]))
        w[np.where(train_pred == train.iloc[:, -1])] = w[np.where(train_pred == train.iloc[:, -1])] * math.exp(
            -alpha[t])
        w[np.where(train_pred != train.iloc[:, -1])] = w[np.where(train_pred != train.iloc[:, -1])] * math.exp(alpha[t])

        w = (w / w.sum())

        train_pred_his = np.append(train_pred_his, [train_pred], axis=0)
        valid_pred_his = np.append(valid_pred_his, [valid_pred], axis=0)

        if t % 9 == 0 and t != 0:
            train_acc.append((np.where(np.dot(train_pred_his.T, alpha) > 0.5, 1, 0) == train.iloc[:, -1]).mean())
            valid_acc.append((np.where(np.dot(valid_pred_his.T, alpha) > 0.5, 1, 0) == valid.iloc[:, -1]).mean())
...
for dmax in [1, 2, 5]:
    w = np.ones(train.shape[0])
    w = w / train.shape[0]
    random_forest(117, dmax, w)

for dmax in [1, 2, 5]:
    fig, ax = plt.subplots()
    with open(r"train_acc_" + str(117) + "_" + str(dmax) + ".pkl", "rb") as f:
        train_acc = pickle.load(f)
    with open(r"valid_acc_" + str(117) + "_" + str(dmax) + ".pkl", "rb") as f:
    ...
    plt.ylabel("Accuracy")
    plt.title("training and validation accuracy with dmax=" + str(dmax))
    plt.savefig("accuracy_" + str(dmax) + ".png")
